...
def masked_softmax(logits: Tensor, mask: Optional[Tensor] = None):
    ...
    if mask is not None:
        if mask.dtype == jnp.bool_:
            ...
            mask = mask.astype(logits.dtype) * .5 * min_value
        else:
            assert jnp.issubdtype(mask.dtype, jnp.floating), f'Expected float tensor for mask, got {mask}'
        logits = logits + mask
...
class MultiheadAttention(Module):
    ...
    def default_config(cls):
        ...
        cfg.define('num_heads', 0, 'Number of attention heads. Must divide hidden_dim evenly.')
        cfg.define('input_linear', MultiheadInputLinear.default_config(),
    ...
    def __init__(self, cfg: config_lib.Config, *, parent: Module):
        ...
        cfg = self.config
        for name, dim in (('q', cfg.query_dim), ('k', cfg.key_dim), ('v', cfg.value_dim), ('o', self.output_dim())):
    ...
    def forward(self, query: Tensor, *, key: Tensor, value: Tensor, mask: Optional[Tensor] = None) -> Output:
...
class TransformerAttentionLayer(Module):
    ...
    def forward(self, *, target: Tensor, source: Optional[Tensor] = None, mask: Optional[Tensor] = None):
...
class TransformerFeedForwardLayer(Module):
    ...
    def default_config(cls):
        ...
        cfg.define('linear_tpl', Linear.default_config(), 'Whether the linear modules have biases.')
...
class TransformerLayer(Module):
    ...
    def default_config(cls):
        cfg = super().default_config()
        cfg.define('input_dim', 0, 'Input feature dim.')
        cfg.define('self_attention', TransformerAttentionLayer.default_config(), 'The self-attention layer config.')
        cfg.define('cross_attention', None, 'If not None, the cross-attention layer config.')
        cfg.define('feed_forward', TransformerFeedForwardLayer.default_config(), 'The feed-forward layer config.')
        return cfg
    ...
    def forward(self, data: Tensor, *, self_attention_mask: Optional[Tensor] = None,
                cross_attention_data: Optional[Tensor] = None, cross_attention_mask: Optional[Tensor] = None) -> Output:
        ...
        if cross_attention_data is not None:
            cross_atten_outputs = self.cross_attention(target=data, source=cross_attention_data,
        ...
        return self.Output(data=data, self_attention_probs=self_atten_outputs.probs,
