...
class DefaultInitTest(absltest.TestCase):
    ...
    def testScale(self):
        init: DefaultInit = DefaultInit.default_config().instantiate()
        scale = init.initialize('scale', prng_key=jax.random.PRNGKey(1), shape=[4], dtype=jnp.bfloat16)
        ...
        for dist in ('uniform', 'normal'):
            for gain in (1., 2.):
                for fan_type in ('fan_in', 'fan_out', 'xavier'):
                    init: DefaultInit = DefaultInit.default_config().set(fan=fan_type, gain=gain,
                                                                         distribution=dist).instantiate()
                    fan = init._get_fan('weight', shape)
                    self.assertEqual(fan, {'fan_in': 8 * 3 * 3, 'fan_out': 4 * 3 * 3, 'xavier': 6 * 3 * 3}[fan_type])
                    weight = init.initialize('weight', prng_key=jax.random.PRNGKey(1), shape=shape, dtype=jnp.float32)
