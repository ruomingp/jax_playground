...
class MetricAccumulator(config.Configurable):
    ...
    def update(self, input_batch: Any, model_outputs: Dict[str, Any]):
        logging.debug("MetricAccumulator.update: current=%s update=%s", self._scalars, model_outputs)
        scalars = self.tree_map(lambda x: x if isinstance(x, WeightedScalar) else tuple(), model_outputs)
