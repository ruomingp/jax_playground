...
class MaskTest(absltest.TestCase):
    ...
    def testSegmentMask(self):
        np.testing.assert_array_equal(
            ...
            attention.make_segment_mask(target_segments=jnp.asarray([[1, 1, 2, 0]]),
...
def _attention_parameters(src: hf_roberta.RobertaSelfAttention):
    ...
    results = {'attention': {}}
    for src_proj, dst_proj in (('query', 'q_proj'), ('key', 'k_proj'), ('value', 'v_proj')):
        ...
        dense_params = dict(weight=dense.weight.transpose(0, 1).view(-1, num_heads, per_head_dim),
    ...
    results['attention']['o_proj'] = dict(
        weight=output_dense.weight.view(-1, num_heads, per_head_dim), bias=output_dense.bias)
...
class _BaseTest(absltest.TestCase):
    ...
    def _compare_attention_layers(self, ref: hf_roberta.RobertaAttention, layer: attention.MultiheadAttention):
        ...
        for mask in (None,):
            ...
            layer_outputs: TransformerLayer.Output = layer(
                target=jnp.asarray(target), mask=mask,
                context=layer.make_invocation_context(parameters=layer_params, is_training=True,
                                                      prng_key=jax.random.PRNGKey(0)))
            attn_mask = None if mask is None else torch.as_tensor(mask).repeat_interleave(num_heads, dim=0)
            ref_outputs, = ref.forward(torch.as_tensor(target, dtype=torch.float32), attention_mask=attn_mask,
...
class TransformerAttentionTest(_BaseTest):
    ...
    def testForward(self):
        ...
        cfg = attention.TransformerAttentionLayer.default_config().set(
            name='test', target_dim=model_dim, source_dim=model_dim, structure='postnorm')
        cfg.attention.set(num_heads=num_heads)
        layer = cfg.instantiate(parent=None)
        roberta_config = hf_roberta.RobertaConfig(hidden_size=model_dim, num_attention_heads=num_heads, attention_probs_dropout_prob=0, hidden_dropout_prob=0, classifier_dropout=0)
