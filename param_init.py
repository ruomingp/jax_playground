...
class Init(config.Configurable):
    ...
    def initialize(self, name: str, *, prng_key: jax.random.KeyArray, shape: Shape, dtype: jnp.dtype) -> jnp.ndarray:
...
class DefaultInit(Init):
    ...
    def default_config(cls):
        ...
        cfg.define('gain', 1., 'The default gain for weight initialization.')
        cfg.define('fan', 'xavier', 'How to compute the fan, supported values are "fan_in", "fan_out", and "xavier".')
    ...
    def _initialize_weight(self, name: str, *, prng_key: jax.random.KeyArray, shape: Shape,
