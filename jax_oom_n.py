...
On the TPU VM:
mkdir -p ./jax_oom_n && python3 jax_oom_n.py --dir=./jax_oom_n 2>&1 | tee ./jax_oom_n/log

Same as jax_oom_m except with explicit deletions.
"""
import os.path
import time
...
class SpmdTrainer(Module):
    ...
    def _run_step(self, prng_key: jax.random.KeyArray, input_batch: Any):
        ...
        with jax.profiler.StepTraceAnnotation("train", step_num=self.step):
            ...
            self._state = outputs.pop("state")
        if self._state.step % 1 == 0:
            self._step_log(
                "loss=%s outputs=%s",
                outputs["loss"],
                jax.tree_map(
                    lambda x: x.item() if x.ndim == 0 else f"T{x.shape}", outputs
                ),
            )
        del outputs
